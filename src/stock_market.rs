...
// use plotters::backend::BackendStyle;
...
impl StockInformation {
     ...
     ***/
    pub fn get_change_of_stock_data_with_given_datetime(
        &self,
        search_datetime: DateTime<Utc>,
    ) -> Option<StockData> {
        ...
        match stock_data_series_with_change {
            Some(stock_data_series_with_change) => {
                return stock_data_series_with_change
                    .into_iter()
                    .find(|stock_data| stock_data.datetime == search_datetime);
            }
            None => None,
        }
    }
    ...
    pub fn show_chart(
        ...
        //     CandleStick::new(
        //         stock_data.datetime.date_naive(),
        ...
        //     stock_data_series[0].datetime.date_naive() - Duration::days(1),
        ...
        let (from_datetime, to_datetime) = (
            stock_data_series[0].datetime - Duration::days(1),
            stock_data_series[stock_data_series_last_day_idx].datetime + Duration::days(1),
        );
        ...
        let x_spec = from_datetime..to_datetime;
        ...
        if ma_days.len() > 0 {
            ...
            for (idx, ma_tuple) in moving_averages_2d.iter().enumerate() {
                ...
                let mut ma_line_data: Vec<(DateTime<Utc>, f64)> = Vec::with_capacity(3);
                ...
                for i in 0..ma_len {
                    ...
                    ma_line_data.push((
                        // stock_data_series[ma_day].datetime.date_naive(),
                        stock_data_series[ma_day].datetime,
